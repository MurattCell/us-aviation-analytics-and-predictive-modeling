...
plt.show()
df_reg_sonuc = pd.DataFrame({'Gercek_Gecikme': y_test, 'Tahmin_Gecikme': y_pred})
df_reg_sonuc.to_csv('regression_results.csv', index=False)
# 7. POWER BI İÇİN ÇIKTI ALMA (Sınıflandırma Sonuçları)
# 'y_test' gerçek durumu, 'y_pred' ise modelin tahminini temsil eder.
df_siniflandirma_sonuc = pd.DataFrame({
    'Gercek_Iptal_Durumu': y_test, 
    'Tahmin_Edilen_Iptal_Durumu': y_pred
})

# Dosya adını 'iptal_tahmin_sonuclari.csv' yaparak kaydediyoruz
df_siniflandirma_sonuc.to_csv('iptal_tahmin_sonuclari.csv', index=False)

print("\n--- BAŞARILI: Power BI için CSV dosyası oluşturuldu! ---")
